...
% Nagel plan:
%
% Look how cheap and fast main memory is, get rid of disks
%
% The application data model is very different than the relational
% model
%
% The system and the code are on the same server and do not need all
% the bells and whistles. => Store data as normal containers (but
% it's hard)
...
\begin{figure}[H]
  \begin{tikzdiagram}
    ...
    \node {\(lineitem\)} % lineitem
    child { \bn{\sigma_{q}(lineitem)}} % σ_q
    child { \bn{\sigma_{\neg q}(lineitem)}} % σ_nq
    child { \n{\sigma_{\neg p}(lineitem)}}  % σ_np
    child {
      \n{\sigma_{p}(lineitem)} % σ_p
      child {\n{\sigma_{p \land r}(lineitem)} } % σ_pq
      child {\n{\sigma_{p \land \neg r}(lineitem) }} % σ_rpq
    } ;
\end{tikzdiagram}
