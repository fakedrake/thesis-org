...
\begin{corrected}{Architecture}
  ...
  graph using an \(A^{\star}\) inspired branch and boud algorithm in
  order to come up with a plan that a) efficiently computes the
  reuqested query and b) optimizes the data layout to provide
  oportinity for efficient future plans. Prunning of the search space
  ...
  wether a relation is materializable at all given the currently
...
\begin{corrected}{Thesis contributions}
  The contributions of this work are summarized by the following
  points:
...
\item We introdice reversible relational operators and how they can be
...
\item We introdicue Antisthenis, a novel approach to incremental
