...
% TODO: Is Tup2 reflected in the text?
\begin{code}
\begin{haskellcode}
...
\end{haskellcode}
  \caption{\label{lst:plant_def}The monad that defines all the useful
    ...
    the QDAG, the n-node sizes, etc. \hask{GCState t n} is state that is
...
nodes are materialized. If the n-node is already materialized the
...
\item The n-node's status found to be \hask{Concrete} and \hask{NoMat}
  ...
  materialized.
\item Tf it is \hask{Concrete} and \hask{Mat} the assertion simply
  succeeds
...
avoid this problem, n-nodes materialized are set to \hask{Concrete Mat}
...
It is worth highlighting the importance of each node in a dependency set of an n-node
...
the \emph{garbage collector} (GC) mechanism selects the least useful nodes to be
...
the form of setting n-nodes to \hask{Concrete Mat}. That is to say, hen a new
...
tryDelete node = do
  -- check that the node is still deletable
  ...
  -- Mark the node as not materialized.
  setStatus node (Initial NoMat)


-- | Check that a node is deletable
isDeletable n = do
  st <- getNodeState n
  case st of
    Initial Mat -> do
...
-- | A node that was not deletable should not be considered again
...
that value is zero, the node is not considered protected.
...
\item The sum of the estimated costs of \emph{each node in the frontier}.
...
materialized nodes. Since the set of materialized nodes does not
...
of estimating cost materializing n-nodes. We incrementally estimate the cost of each node
...
stuck behind materialized nodes: past queries, espsecially recent ones
...
\item We do not care about the cost of the particular nodes, but rather
  about \emph{similar} nodes. For that reason we want to avoid
  depending too heavily on a particular materialized node being
  materialized.
\end{itemize}

A more realistic way of calculating the expected cost that an n-node will
...
to get the stochastic cost. Thus the cost formula \(h\) for the nodes
now is:
