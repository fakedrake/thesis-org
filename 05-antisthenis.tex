...
value of type \hask{s}. Cpomposing two Kleisli arrows is equivalent to
...
The concepts that we will dub a \emph{mealy arrow} or \emph{mealy
...
\todo{Clarify} Where \(e \in \mathit{ZEpoch w}\),
\(c_0,c_1 \in \mathit{ZCoEpoch w}\) and \(\diamond\) is the monoidal
merging operation on coepochs.
...
special case for the cap is \hask{ForceResult} for which
...
\hask{ArrProc w m}, etc). We refer to these types as ``tags''. We have
...
expression \hask{False}. Or more succintly
...
\ref{subfig:comp_d_naive_TODO_not_ref} where naive-Antisthenis looks
...
by the evaluation process in figure \ref{fig:comp_d_smart} (the reader
