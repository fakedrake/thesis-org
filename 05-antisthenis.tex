...
\end{haskellcode}
\label{lst:maybe_def}
\caption{Definition of the \hask{Maybe} monad.}
\end{code}
...
\caption{Example usage of the \hask{Maybe} applicative functor.}
\end{code}
...
\end{haskellcode}
\label{lst:arrow_def}
\caption{Haskell typeclasses related to the notion of \hask{Arrow}.}
\end{code}
...
\caption{The \hask{MB a b m} monad can be used as a convenience to
...
\hask{ArrProc}. What this means in practice is that due to the writer
...
Where \hask{e :: ZEpoch w}, \hask{c0,c1 :: ZCoEpoch w} and \hask{<>} is the monoidal
...
In more concrete terms monotonicity is indicated by the following
equation:
...
\end{haskellcode}
\label{lst:mkproc}
\caption{The type \hask{w} fully defines the operator so combining
...
\end{haskellcode}
\label{lst:conv_def}
\caption{An object of type \hask{Conv w w'} can act as an interface
  between parent processes of type \hask{w} to subprocesses of type
  \hask{w'}.}
\end{code}
...
\begin{figure}[H]
TODO
% \begin{tabular}{|p{\colw}|p{\colw}|}
%   getCost[A] & getCost[D] (bad) \\
%   \hline
%   \comp{\bot}{A}{
%     comp[B] \ev 10 \\
...
\begin{figure}[H]
  TODO
% \begin{tabular}{|p{\colw}|p{7cm}|}
%   getCost[A] & getCost[D] (good) \TBstrut\\
%   \hline
%   \comp{\bot}{A}{
%     comp[B] \ev 10 \\
%     \comp{\bot}{C}{comp[A] \ev \bot}}
...
\todo{THE CAP COMPARISON IS WRONG}
