...
\dictum[Plato]{You have the eye which sees a horse and a man; but the
  eye which can see horsehood and manhood you have not.}
...
and once that is evaluated, the evaluation of \(A\) is complete\footnote{Barring domains where infinity needs to be considered in any way}. In a
...
\hask{ZipperParams} token because, as we will be seeing throughout the
chapter, it mainly parameterize the evolution of the internal state of
the Antisthenis process, the \hask{Zipper}. We will be looking in
detail into the methods that \hask{ZipperParams} tag needs to
...
\hask{ZCoEpoch}.  It is a datastructure that summarizes the provenance
...
not yield \hask{BndRes 0} and thereby we would need to evaluate
...
\item A set of \hask{iteration} processes, processes whose latest
  ...
  \hask{initial} processes bounds returned. Since these processes
...
evaluated, and their iterations are then moved from the \hask{initials}
set to the \hask{coiterations} set, possibly with an intermediate stop
in the \hask{iterations} set according to the values and
...
\hask{iterator set} is highly operator dependent and primarily geared
...
\hask{Cmds} is a union type encapsulating different directions to which the
zipper can be evolved. Its definition is provided in listing
\ref{lst:cmds_def}. The point of \hask{Cmds} is to separate the logic of
...
\hask{ZProc} is therefore a process that simulataneously encapsulates all possible evolution paths
for the zipper.
...
The \hask{Cmds} functor facilitates a tree of actions in conjunction with a
free monad.  A free monad of functor \hask{f} is actually many
...
In subsection \ref{sec:cmds_functor} discussing the \hask{Cmds} functor we
...
\hask{False} to take advantage of early termination due to absorbing elements.
...
\hask{ArrProc w m}, etc). We refer to these types as \emph{tags}. We have
...
is a completely generic process
\hask{mkProc} (see listing \ref{lst:mkproc}) which combines a list of subprocesses into a parent process
that evaluates the operator described by the tag \hask{w} without
requiring any further information other than the implementation of \hask{ZipperParams}.
...
other options are more expensive than 7 steps for \hask{True} and 4
for \hask{False}''. It could be seen as a metric for the size of the
tree.
...
Since machines are referable by different processes it means
...
Another way to look at predicates is as deferred parts of the coepoch and
