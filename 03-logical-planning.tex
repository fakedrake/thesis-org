\dictum[F. Kafka -- The Cares of a Family Man]{At first glance
[Odradek] looks like a flat star-shaped spool for thread, and indeed
it does seem to have thread wound upon it; to be sure, they are only
old, broken-off bits of thread, knotted and tangled together, of the
most varied sorts and colors. [...] He does no harm to anyone that one
can see; but the idea that he is likely to survive me I find almost
painful»}
...
\begin{corrected}{Chapter contributions}
  The contributions described in this chapter center around the use of
  reversible operations for planning queries in the presence of
  materialized results, as well as the conditions these create for
  materialized view selection algorithms. The particulars of the
  latter will be described in detail in chapter
  \ref{chapter:physical_planning}.
\end{corrected}
...
breaking the excluded middle axiom. This allows the definition of a
...
aggregation functions and their names, and the second argument is the
...
values as they break Boolean excluded middle. Furthermore, relations
...
the end of ensuring the materialisability of only \(A\) and not
\(B\).
...
\item Our parser implements \hask{MonadGen e p} which means it can generate a value of type \hask{p
    e}  and does not consume any input. Useful for creating names for unnamed expressions in
  projections, groups. The function reifying this feature is called \hask{mkSymbol}.
\end{itemize}
...
Once the preliminary sanitisation and simple optimization steps are
...
\paragraph{Remapping Unique subtuples}
...
enough to come up with accurate types as SQL's type system is fairly simple.

\paragraph{Shared unique subtuple annotation on projections}
...
Optimisers that process queries in isolation can aggressively push
...
modification of the individual fields so that the \hask{cnfColumns} vector can
...
In each case, the columns themselves are stored in a container of parametric type \hask{f}, which is
...
\ref{lst:qnf_name}), the atoms of predicates and other expressions
(see section \ref{sec:expressions_predicates}).  A name in the context
of QNFs can be one of three different kinds:

\begin{itemize}
\item \textbf{A column of a non-primary relation.} This relation
...
\item \textbf{A column of a primary table}. These columns have a name
  ...
  product collection like in the case of non-primary columns. It is
  indexed much like the non-primary relation.
...
primary columns in \hask{QNFSelName}. This has to do with the way
...
we apply the operations incrementally to a named QNFs (NQNF).  Applying
...
\begin{itemize}
\item A \hask{ListT} monad transformer allows for non-deterministic computation in
...
\item Since the resulting QNF is a highly self-referential structure,
  ...
  shared caching between QNF computations, FluiDB would be repeatedly computing the QNF of the
...
Every QNF building function accepts one or two \hask{NQNFQuery} forms and a
...
\item The RA operator that was provided as input with the expression atoms
  ...
  referring to the input or output columns. The way the names in the operator expressions are translated to
...
\end{pycode}
  \caption{\label{lst:qnf_product}The QNF product algorithm finds name conflicts between the
    ...
    FluiDB non-determinism is handled by the \hask{ListT} monad.}
\end{code}
...
\item \(A \lnsemi B = B \rnsemi A \) \textit{(via explicit enumeration in \hask{QNFProd})}
\item \(A \cup B = B \cup A \) \textit{(via explicit enumeration in \hask{QNFProd})}
\end{itemize}
...
both refer to some transformation of a column labeled \(a\). On the
...
correlating subsystem, or any other query processing step. \emph{Otherwise
...
Thereby, each n-node is coupled with a structure containing all relevant
...
\begin{figure}[H]
  ...
  \begin{subfigure}{0.9\textwidth}
    ...
    \caption{\label{fig:joinclust_fin}The structure of a \hask{JoinClust}
  ...
  \caption{\label{fig:joinclust}There are four cluster types. The
    \hask{NClust} is trivial, just a single n-node. The other 3 are
...
synchronize} a cell before triggering the propagator to make sure we
...
propagators is copying the default value of a non-full propagator to
