...
no \sql{NULL} values are allowed since a predicate over a NULL value
always evaluates to \hask{False}. Further we assume set semantics for all
...
\begin{itemize}
\item The top layer is purely boolean operations \hask{Prop}. This
...
\item The lowest layer \hask{Expr} is expressions of any kind that may
  ...
  layer because in order to support the ternary operator \hask{if
...
parser \hask{p} has the form \hask{p a} which means that if the parser is used
to parse matching text, it will return a result of type \hask{a}. For this
framework to work two combinators are required:

\begin{itemize}
\item seqnence (the monadic bind) \hask{(>>=) : p a -> (a -> p b) -> p b}
  which sequentially tries two combinators and the latter one may use
  the result of the former.
\item the 'alternative' parser combinator \hask{<|> : p a -> p a -> p a} which
  tries one parser and if that fails tries another.
\item \hask{MonadGen e p} can generate a value of type \hask{p e}. Useful for creating
  names for unnamed expressions in projections, groups.
\end{itemize}

Simple parsers like the \hask{parseInt} parser (listing \ref{org463a483})
...
to be applied a very simple product query generated by the \sql{from}
clause.}
\end{code}
...
\item Date intervals are translated to dates. This is because the plysical
  ...
  exaple the expression \sql{date '2020-12-01' - interval '90' day} is
  immediately turned into \sql{date 2021-03-01}. Note that an expression
  like \sql{date '2020-12-01' + shipping\_time} is not supported as it is
  not date arithmetic between literals.
\item Optimization of the \sql{like} operator: \sql{<VAR> like
    <STRING\_LITERAL>} expressions where \sql{<STRING\_LITERAL>}
  begins or ends with \sql{\%} are turned into \hask{suffix\_of} and
  \hask{prefix\_of} expressions. For example the expression \sql{a
    like "\%.txt"} would be turned into \sql{".txt".suffix\_of(a)}.
...
Whenever a \hask{QNFQuery} is found outside of the QNF package \hask{col\_f} will
be instantiated as \hask{Either}, which is to say that a query may be an
aggregation (\hask{Right}) or a projection (\hask{Left}). When there is no
explicit projection at the top of a query, the columns are simply
enumerated. While the \hyperref[sec:relational_algebra_semantics]{RA} is fundamentally named, \hask{QNFQuery} instances
are unnamed. Instead we use a \hask{QNFColumn} in place of names. With that
...
any ordering. The \hask{Expr} however is the same. FluiDB makes no attempt
...
syntactic representations. The query \sql{select * fom A as A1, A as A2
  where A1.a = A2.a} is symmetric in all repsects but the query
\sql{select * fom A as A1, A as A2 where A1.a = A2.a + 1} is not. Indexing
\sql{A1} and \sql{A2} differently does not change the semantics of the query
...
\ref{org3c5bf71}). However they only operate on the non-recursive \hask{Aggr}
algebra, while \hask{QGroup} uses \hask{Expr (Aggr (Expr e))}. QNF moves the
inner and outer \hask{Expr} to one level up and one level down
...
\begin{code}
  ...
  \end{haskellcode}
  \caption{\label{org3c5bf71}The QNF aggregation form of the projection field is similar to projection only, much like the \hask{QGroup} constructor, it also includes a hashset of exprssions on which to group.}
\end{code}
...
The \hask{qnfSel :: sel\_f (QNFSel e s)} field contains an exprssion of the
selection (see listing \ref{orgc76c888}). With very few exceptions \hask{sel\_f}
...
It needs to be pointed out that \emph{despite} the fact that the \hask{qnfSel}
field refers to output columns the order of operations in the sematics
of a QNF is \(\pi\sigma(..)\) rather than \(\pi\sigma(..)\). In the
case where \hask{cnfCol} describes a projection the selection can be pushed
down. In the case, however, where \hask{cnfCol} describes an aggregation
...
The other datastructure is the \hask{qnfHash :: QNFKey}. Because the QNF is
...
RA form queries FluiDB save on memory and use \hask{QNFKey} in place of the
\hask{QNFQuery} structure (see listing \ref{org491a01e}). On the other hand,
...
that the user would be consistent when naming their columns (\sql{select
...
\begin{itemize}
\item \hask{NClust} is a cluster containing exactly one slot which corresponds
  to a primary table.
\item \hask{UnClust} a cluster corresponding to a unary operation. It has two
  slots in output position, one slot in input position, no
  intermediate nodes and a single t-node slot \ref{fig:org8d8d9a2}.
\item \hask{BinClust} is a cluster corresponding to a non-join binary RA
  operation. It has one output slot, two input slot and a t-node slot
  \ref{fig:org71a57f1}.
\item \hask{JoinClust} augments the \hask{BinClust} with two intermediate nodes, two
...
\begin{figure}[H]
  ...
  \caption{\label{fig:org8d8d9a2}The structure of a \hask{UnClust}.}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{\label{fig:org71a57f1}The structure of a \hask{BinClust}.}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{\label{fig:orgf1d2858}The structure of a \hask{JoinClust}
...
look up. We memoize the entire process using the \hask{qfHash} to avoid
...
\hask{Nothing}, the middle level is \hask{Just x} and the top
